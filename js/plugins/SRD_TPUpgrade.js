...
Game_Action.prototype.executeTPDamage = function(target, critical) {
	var damage = 0;
	var GUILTY_STATE = 16;
	var SKEPTICAL_STATE = 11;
	var SENSITIVE_STATE = 6;
	try {
        var item = this.item();
        var a = this.subject();
        var b = target;
	...
	//damage = this.applyVariance(damage, 0); b.isStateAffected(2) ? a.mdf*200 * 8 - b.def * 2 : a.mdf*200-b.def
	damage = this.applyGuard(damage, target);
	damage = Math.round(damage);
	this.makeSuccess(target);
	if(b.isStateAffected(SENSITIVE_STATE)){ 
		damage = damage*2;
	}else if(b.isStateAffected(SKEPTICAL_STATE)){
		damage = damage*-1;
	}else if(b.isStateAffected(GUILTY_STATE)){
		damage = damage*2;
	}else{
		damage = damage;
	}
