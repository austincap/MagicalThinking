...
Enemy_Bars.prototype.tpgaugeColor = function(rate) {
	if(rate > 0.75){
		return low;//High hp color
	}else if(rate > 0.50){
		return medium;//Medium hp color
	}else if(rate > 0.25){
		return high;//Low hp color
	}else{
		return high;//Critical hp color
	};
};
...
Enemy_Bars.prototype.drawBar = function() {
	...
	for(var i = 0; i < $gameTroop._enemies.length; i++){
		...
		var currentTp = $gameTroop._enemies[i]._tp;
		...
		if(currentTp > 0 && enabled){
			//Max hp defined from the enemies parameters.
			//console.log($dataEnemies[enemyId]);
			//var maxTp = $dataEnemies[enemyId].params[0];
			var tprate = currentTp/$dataEnemies[enemyId].tpu_mtp;
			//Rate is the currnet hp compared to the max hp
			if (tprate > 0.85 && !$gameTroop._enemies[i].isStateAffected(7)){
				console.log("REALLY PISSED OFF");
				$gameTroop._enemies[i].addState(7);
				// SceneManager._scene._logWindow.addText($dataEnemies[enemyId].battlerName+" is really pissed off!");
